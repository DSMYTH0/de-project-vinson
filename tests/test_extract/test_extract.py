...
from extract.extract.extract_lambda import extract_handler
from extract.extract.fetch_table_list import fetch_table_names
from extract.extract.get_last_extract import get_last_extracted_time
from extract.extract.update_extract import update_extracted_time
from extract.extract.extract_data import extract_data
from extract.extract.put_csv import put_csv
...
class TestUtilFunctions:
    ...
    def test_function_checks_the_database_is_empty(self):
        #patch connection
        with patch('extract.extract.extract_lambda.connect_to_db') as mock_connection:
    ...
    def test_function_returns_expected_dataframe(self):
        ...
        with patch('extract.extract.extract_lambda.connect_to_db') as mock_connection:
            with patch('extract.extract.extract_data.get_last_extracted_time') as mock_extract_time:
                with patch('extract.extract.extract_data.update_extracted_time') as mock_update_time:
    ...
    def test_function_returns_expected_on_first_execution(self, s3_bucket, sample_dataframe):
        ...
        with patch('extract.extract.extract_lambda.connect_to_db') as mock_connection:
            with patch('extract.extract.extract_lambda.fetch_table_names') as mock_fetch:
                with patch('extract.extract.extract_lambda.extract_data') as mock_extract_data:
                    with patch('extract.extract.extract_lambda.put_csv') as mock_put_csv:
                        
                        with patch('extract.extract.extract_lambda.datetime') as mock_datetime:
    ...
    def test_function_returns_expected_on_second_execution(self, s3_bucket, sample_dataframe, sample_small_dataframe):
        ...
        with patch('extract.extract.extract_lambda.connect_to_db') as mock_connection:
            with patch('extract.extract.extract_lambda.fetch_table_names') as mock_fetch:
                with patch('extract.extract.extract_lambda.extract_data') as mock_extract_data:
                    with patch('extract.extract.extract_lambda.put_csv') as mock_put_csv:
                        with patch('extract.extract.get_last_extract.create_new_extract') as mock_create_new:
                            with patch('extract.extract.extract_lambda.datetime') as mock_datetime:
