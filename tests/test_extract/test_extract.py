...
from extract.extract.extract import extract_handler
...
class TestUtilFunctions:
    ...
    def test_function_checks_the_database_is_empty(self):
        #patch connection
        with patch('extract.extract.extract.connect_to_db') as mock_connection:
    ...
    def test_function_returns_expected_on_first_execution(self, s3_bucket, sample_dataframe):
        ...
        with patch('extract.extract.extract.connect_to_db') as mock_connection:
            with patch('extract.extract.extract.fetch_table_names') as mock_fetch:
                with patch('extract.extract.extract.extract_data') as mock_extract_data:
                    with patch('extract.extract.extract.put_csv') as mock_put_csv:
                        
                        with patch('extract.extract.extract.datetime') as mock_datetime:
